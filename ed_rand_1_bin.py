...
def ed_rand_1_bin(np, max_gen, f, cr):
    ...
    for g in range(max_gen):
        ...
        for i in range (np):
            ...
            population = population[order]
        logging.debug("Current best internal CV score: %s" % (fitness[0]))
        # Surplus disposal
        population = population[:np]
        fitness = fitness[:np]
    logging.debug("Best individual is [alpha=%s, fit_prior=%s], fitness=%s" % (alphas[0], False if fit_priors[0] < 0.5 else True, fitness[0]))
    return fitness[0]
...
if __name__ == "__main__":
    ...
    X_train, X_test, y_train, y_test = train_test_split(X_train, y_train, train_size=0.80, test_size=0.20, random_state=7)
